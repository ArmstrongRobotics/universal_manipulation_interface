                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        with torch.no_grad():
                            ...
                            print("Euler angles (yaw, pitch, roll):", euler)
                            import matplotlib.pyplot as plt
                            from mpl_toolkits.mplot3d import Axes3D

                            actions = pred_action[0].cpu().numpy()  # shape: (16, 10)
                            img_tensor = batch['obs']['camera0_rgb'][0, 1]  # shape: (3, 224, 224)
                            ...
                            for j in range(0, 2):
                                ...
                                for i, pose in enumerate(temp):
                                    ...
                                    v1 = pose[3:6]
                                    v2 = pose[6:9]
                                    # 2. Get first orthogonal vector (b1)
                                    v1 = v1 / np.linalg.norm(v1)

                                    # 3. Get second orthogonal vector (b2)
                                    # Subtract the component of a2 that is in the direction of b1
                                    v2 = v2 - np.dot(v1, v2) * v1
                                    v2 = v2 / np.linalg.norm(v2)

                                    # 4. Get third orthogonal vector (b3)
                                    v3 = np.cross(v1, v2)

                                    if(j==0):
                                        scale = 0.03
                                        ax1.scatter(*pos, color='k')
                                        ax1.text(pos[0], pos[1], pos[2], f't={i}', fontsize=8, color='purple')
                                        ax1.quiver(*pos, *(v1*scale), color='r')
                                        ax1.quiver(*pos, *(v2*scale), color='g')
                                        ax1.quiver(*pos, *(v3*scale), color='b')
                                        ax1.quiver(*pos, *([.03, 0, 0]), color='y')
                                        ax1.quiver(*pos, *([0, 0.03, 0]), color='y')
                                        ax1.quiver(*pos, *([0, 0, 0.03]), color='y')
                            ...
                            ax1.set_ylabel('Y')
                            ax1.set_zlabel('Z')
                            ax1.set_title('16-step Action Poses with Timestamps')

                            # Image plot
                            ax2 = fig.add_subplot(1, 2, 2)
                            ax2.imshow(np.clip(img_np, 0, 1))
                            ax2.set_title('Camera Image')
                            ax2.axis('off')

                            plt.tight_layout()
                            plt.show()
                      #  input("Press Enter to continue batch: " + str(batch_idx))  # --- IGNORE ---
